CREATE OR REPLACE FUNCTION adempiere.invoicepaidnegttype(p_c_invoice_id numeric, p_c_currency_id numeric, p_multiplierap numeric, p_typenegotiation  character varying)
 RETURNS numeric
 LANGUAGE plpgsql
 STABLE
AS $function$
	...
	LOOP
		v_PaymentAmt := v_PaymentAmt
			+ currencyConvert(ar.Amount + ar.DisCountAmt + ar.WriteOffAmt,
				ar.C_Currency_ID, p_C_Currency_ID,(CASE WHEN p_typenegotiation='DD' THEN v_DateInvoiced ELSE ar.DateTrx END), null, ar.AD_Client_ID, ar.AD_Org_ID);
	END LOOP;
