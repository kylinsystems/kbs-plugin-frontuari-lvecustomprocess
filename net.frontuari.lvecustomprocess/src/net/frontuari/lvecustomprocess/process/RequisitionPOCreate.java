...
public class RequisitionPOCreate extends SvrProcess
{
	 ...
	 * 	@throws Exception
	 */
	private void newOrder(MRequisitionLine rLine, int C_BPartner_ID, int docTypeId) throws Exception
	{
		if (m_order != null)
		{
			closeOrder();
		...
		if (m_order == null)
		{
			...
			if(docTypeId>0)
				m_order.setC_DocTypeTarget_ID(docTypeId);
			else
	...
	private void newLine(MRequisitionLine rLine) throws Exception
	{
		...
		String sql = "SELECT C_DocTypeOrder_ID FROM C_DocType WHERE C_DocType_ID="+requisition.getC_DocType_ID();
		int docTypeTragetId = DB.getSQLValue(get_TrxName(), sql);
		...
		{
			newOrder(rLine, C_BPartner_ID, docTypeTragetId);
		}
